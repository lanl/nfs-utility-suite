    ...
    pub(super) fn deserialize_definition_zcopy(
        ...
        buf.code_block(
            ...
            |buf| {
                ...
                match &self.body {
                    ValidatedUnionBody::Bool(b) => b.deserialize_bool_zcopy(buf, tab, size_tab),
                    ValidatedUnionBody::Enum(e) => {
                        e.deserialize_enum_zcopy(&self.get_type_zcopy(tab), buf, tab, size_tab)
                    }
                };
            },
        );
...
impl ValidatedUnionBoolBody {
    ...
    pub(super) fn deserialize_bool_zcopy(
        &self,
        buf: &mut CodeBuf,
        tab: &ValidatedSymbolTable,
        size_tab: &SizeTab,
    ) {
        if let Some(cache_name) = self.cached_name(tab, size_tab) {
            buf.add_line(&format!("{cache_name}.get().map(|v| v.clone())"));
            return;
        }
        ...
        buf.code_block("match has_val", |buf| {
            ...
            buf.code_block("_ =>", |buf| {
                buf.block_statement("let val = ", |buf| {
                    ...
                    buf.add_line("let _input = &self.buf[off..];");
                    self.true_arm.deserialize_inline_zcopy(buf, tab, size_tab)
                });
    ...
    pub(super) fn get_size_inline_bool_zcopy(
        ...
        buf.code_block("Ok(4usize + match has_val", |buf| {
            buf.add_line("0 => Ok(0),");
            buf.code_block("_ =>", |buf| {
                self.true_arm.get_size_inline_zcopy(
                    buf,
                    tab,
                    size_tab,
                    fallible_parent,
...
impl ValidatedUnionEnumBody {
    ...
    pub(super) fn deserialize_enum_zcopy(
        ...
        buf.add_line("let discriminant = xdr_lib::get_i32_immut(_input);");
                    ...
                    match &arm.1 {
                        ...
                        Declaration::Named(n) => {
                            ...
                            buf.block_statement("let inner =", |buf| {
                                if n.is_varlen_reader(tab, size_tab) {
                                    buf.add_line(&format!(
                                        "self.{}.get().unwrap().clone()",
                                        n.name
                                    ));
                                } else {
                                    n.deserialize_inline_zcopy(buf, tab, size_tab);
                                }
                            });
