    ...
    T: Reader<'a>,
{
    ...
    fn next(&mut self) -> Option<T> {
        ...
        let has_val = get_i32_immut(&self.buf[self.off..]);
        ...
        if let Ok(ret) = ret {
            ...
            self.err = Some(DeserializeError);
            None
        }
    }
}
