...
fn test_opaque_arrays_minsize() {
    ...
    let reader = OpaqueArraysReader::new(data.as_slice()).unwrap();
...
fn test_opaque_arrays() {
    let mut data: Vec<u8> = vec![0xF, 0xF, 0xF, 0xF];
    data.extend([0x0, 0x0, 0x0, 0x2]);
    ...
    data.extend([0xF, 0xF]); // padding
    data.extend(1023u32.to_be_bytes());
    data.extend(bytes3.as_slice());
    data.extend([0xF]); // padding
...
fn test_opaque_arrays_reader() {
    ...
    let reader = OpaqueArraysReader::new(&data).unwrap();
...
fn test_an_int_reader() {
    ...
    let reader = AnIntReader::new(&data).unwrap();
...
fn test_int_arrays_reader() {
    ...
    assert_eq!(limited_iter.next().unwrap().unwrap().get_a(), 20);
    assert!(limited_iter.next().is_none());

    let mut unlimited_iter = reader.get_unlimited();
    assert_eq!(unlimited_iter.next().unwrap().unwrap().get_a(), 100);
    assert!(unlimited_iter.next().is_none());
...
fn test_strings_reader() {
    ...
    let reader = StringsReader::new(&data).unwrap();
...
fn test_const_size_array_reader() {
    ...
    let reader = ConstSizeArrayReader::new(&data).unwrap();
    assert_eq!(reader.get_bytes(), &[10, 20, 30, 40, 50]);

    let mut ints_iter = reader.get_ints();
    assert_eq!(ints_iter.next().unwrap().unwrap(), 10);
    assert_eq!(ints_iter.next().unwrap().unwrap(), 20);
    assert_eq!(ints_iter.next().unwrap().unwrap(), 30);
    assert_eq!(ints_iter.next().unwrap().unwrap(), 40);
    assert_eq!(ints_iter.next().unwrap().unwrap(), 50);
...
fn test_many_ints_reader() {
    ...
    let reader = ManyIntsReader::new(&data).unwrap();

    let mut first_iter = reader.get_first();
    assert_eq!(first_iter.next().unwrap().unwrap(), 0x1111222233334444);
    assert_eq!(first_iter.next().unwrap().unwrap(), 0x5555666677778888);
    assert!(first_iter.next().is_none());

    let mut second_iter = reader.get_second();
    assert_eq!(second_iter.next().unwrap().unwrap(), 500);
    assert_eq!(second_iter.next().unwrap().unwrap(), 600);
    assert!(second_iter.next().is_none());

    let mut third_iter = reader.get_third();
    assert_eq!(
        third_iter.next().unwrap().unwrap(),
        0x9999999999999999_u64 as i64
    );
