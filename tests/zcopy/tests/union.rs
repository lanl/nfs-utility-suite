...
fn test_plant_reader() {
    ...
    let reader = PlantReader::from_buf(&buf).unwrap();
...
fn test_num_leaves_reader() {
    ...
    let reader = NumLeavesReader::from_buf(&buf).unwrap();
...
fn test_maybe_a_plant_kind_reader() {
    let buf = to_be_bytes_i32(0);
    let reader = MaybeAPlantKindReader::from_buf(&buf).unwrap();
    assert_eq!(reader.deserialize(), None);
    assert_eq!(reader.get_width().unwrap(), 4);
...
fn test_stuff_reader() {
    ...
    let reader = StuffReader::from_buf(&buf).unwrap();
...
fn test_maybe_stuff_reader() {
    let buf = to_be_bytes_i32(0);
    let reader = MaybeStuffReader::from_buf(&buf).unwrap();
    assert_eq!(reader.deserialize(), None);
    assert_eq!(reader.get_width().unwrap(), 4);
...
fn test_has_string_reader() {
    let buf = to_be_bytes_i32(0);
    let reader = HasStringReader::from_buf(&buf).unwrap();
    assert_eq!(reader.deserialize(), None);
    assert_eq!(reader.get_width().unwrap(), 4);
...
fn test_stuff_or_plant_reader() {
    ...
    let reader = StuffOrPlantReader::from_buf(&buf).unwrap();
...
fn test_same_width_different_stuff_reader() {
    ...
    let reader = SameWidthDifferentStuffReader::from_buf(&buf).unwrap();
    assert_eq!(reader.get_width().unwrap(), 12);
}
...
fn test_stuff_or_plant2_reader() {
    ...
    let reader = StuffOrPlant2Reader::from_buf(&buf).unwrap();
